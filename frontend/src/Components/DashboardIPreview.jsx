...
const DashboardPreview = () => {
  ...
  useEffect(() => {
    ...
    // Check if search parameters were passed from home page
    ...
    if (searchDestination && searchDepartureDate) {
      ...
      // Store trip type filter if provided
      if (searchTripType) {
        setTripTypeFilter(searchTripType);
      }
      
      // Store budget filter if provided
      if (searchBudget) {
        setBudgetFilter(searchBudget);
      }
      
      // Automatically trigger search and scroll to results
      setSearchClick(true);
      setTimeout(() => {
        scrollToDiv();
      }, 300);
  ...
  const FilteredFlights = Flights.filter((item) => {
    ...
    // Check if trip type matches (if specified)
    const tripTypeMatch = !TripTypeFilter || 
    ...
    // Check if budget filter matches (if specified)
